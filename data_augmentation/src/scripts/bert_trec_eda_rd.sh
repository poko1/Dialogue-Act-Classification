...
TASK=da_100 #imdb_whole #imdb #trec
BERTLR=4e-5
...
do
    for i in {0..14}; ##{0..14};
      ...
      EDADIR=$RAWDATADIR/eda_rd
      
      #mkdir $EDADIR  ## creates an eda directory for each segment of the dataset
      
      python $SRC/bert_aug/eda_rd.py --input $RAWDATADIR/train.tsv --output $EDADIR/eda_aug.tsv --num_aug=1 --alpha=0.1 --seed ${i} ## applies EDA on train data segment to generate ada_aug
      
      cat $RAWDATADIR/train.tsv $EDADIR/eda_aug.tsv > $EDADIR/train.tsv ### ori + aug data in new dataset called train.tsv under eda dir of each data segment
      ...
      python $SRC/bert_aug/bert_classifier.py --task $TASK --data_dir $EDADIR --seed ${i} --learning_rate $BERTLR --cache $CACHE  > $RAWDATADIR/bert_eda_rd.log
