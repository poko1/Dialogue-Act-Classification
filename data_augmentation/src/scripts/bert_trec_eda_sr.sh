...
do
    for i in {0..14}; ##{0..14};
      ...
      EDADIR=$RAWDATADIR/eda_sr
      
      mkdir $EDADIR  ## creates an eda directory for each segment of the dataset
      
      python $SRC/bert_aug/eda_sr.py --input $RAWDATADIR/train.tsv --output $EDADIR/eda_aug.tsv --num_aug=1 --alpha=0.1 --seed ${i} ## applies EDA on train data segment to generate ada_aug
      ...
      python $SRC/bert_aug/bert_classifier.py --task $TASK --data_dir $EDADIR --seed ${i} --learning_rate $BERTLR --cache $CACHE  > $RAWDATADIR/bert_eda_sr.log
